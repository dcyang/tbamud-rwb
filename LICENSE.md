...
language, of TbaMUD (The Builder's Academy MUD).  TbaMUD is itself a
derivative of CircleMUD, which is in turn a derivative of DikuMUD.
Because tbamud-rwb reproduces the gameplay of those systems and ships
their world data, it is a derivative work of all three predecessors and
is therefore bound by their licenses, the full texts of which are
reproduced below and form an inseparable part of this license.

This license is intended to be **as permissive as those predecessor
licenses allow** -- no more restrictive than necessary, and no looser
where the predecessors impose a constraint.  In plain terms:

You MAY, free of charge:

- Use, run, and play tbamud-rwb for any non-commercial purpose.
- Read, study, and learn from the source code.
- Modify the source code and run your modified version.
- Copy and redistribute the original or your modified version, in
  source or binary form, provided you carry this license with it.
- Build other works (including other MUDs) on top of tbamud-rwb.

You MUST, because the predecessor licenses require it:

- Not charge money for, sell, or otherwise commercially profit from
  tbamud-rwb or any part of it -- this includes the usual "fee for the
  disk/download," pay-to-win donations, or any compensation for running
  or distributing it.  (CircleMUD condition 1; DikuMUD rules.)
- Preserve every copyright, authorship, and licensing notice found in
  the source code, data files, and this document.  Do not remove or
  alter the names of the DikuMUD, CircleMUD, or TbaMUD authors.
- Keep crediting the predecessors: the DikuMUD and CircleMUD creators
  must appear in the login sequence, the 'credits' text must be kept
  intact, and the "CIRCLEMUD" help entry must remain available, exactly
  as the predecessor licenses demand.
- Distribute this entire license file AS IS with any copy or portion
  you pass on, including derivative works.

You DO NOT need to:

- Ask permission to use, modify, or redistribute, so long as the
  conditions above are met.
- Contribute your modifications back (though contributions are welcome).
- Pay anyone, register anywhere, or accept any additional restriction.

The Rust code original to this rewrite is provided WITHOUT WARRANTY of
any kind; to the maximum extent permitted, the contributors disclaim all
liability arising from its use.

In short: do anything you like with it, for free, but keep it free, keep
the credits, and carry the licenses below.  Where this summary and the
predecessor licenses below disagree, the predecessor licenses govern.

---

# TbaMUD (The Builder's Academy)

TbaMUD (The Builder's Academy MUD) is the CircleMUD distribution from
which this rewrite's gameplay and world data are derived.  It carries no
license of its own beyond the CircleMUD and DikuMUD licenses reproduced
below, and is maintained in the same spirit.
